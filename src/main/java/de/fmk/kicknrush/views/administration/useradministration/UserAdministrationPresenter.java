package de.fmk.kicknrush.views.administration.useradministration;

import de.fmk.kicknrush.helper.UTF8Resources;
import de.fmk.kicknrush.models.administration.useradministration.UserAdministrationModel;
 ...
 * @author FabianK
 */
public class UserAdministrationPresenter implements ISettingsPresenter, Initializable {
	...
	@Inject
	private UserAdministrationModel model;
	...
	private void initList() {
		...
		userList.getSelectionModel().selectedItemProperty().addListener((observable, wasSelected, isSelected) -> {
			...
			if (wasSelected != null && (resources.get(NEW_USER).equals(nameInput.getText()) ||
	...
	private void onCreate() {
		...
		user = model.createUser(resources.get(NEW_USER));
	...
	private void onSave() {
		...
		if (resources.get(NEW_USER).equals(nameInput.getText()) ||
				nameInput.getText() == null || nameInput.getText().isEmpty()) {
